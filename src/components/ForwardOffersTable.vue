  ...
  <section class="card overflow-hidden">
      ...
      >
        <table class="w-full min-w-max divide-y divide-primary-100">
          ...
          <tbody class="divide-y divide-primary-50 bg-white">
            <tr v-for="(offer, index) in forwardOffers" :key="offer.id || index" class="hover:bg-primary-50/40">
              <td class="table-cell max-w-[7rem] overflow-hidden px-4 py-2.5">
              ...
              <td class="table-cell max-w-[7rem] overflow-hidden px-4 py-2.5 text-sm">
                <span class="block min-w-0 truncate font-semibold text-ink" :title="offer.vintages">
                  {{ offer.vintages || "" }}
                </span>
              </td>
              <td class="table-cell max-w-[7rem] overflow-hidden px-4 py-2.5 text-sm">
                <span class="block min-w-0 truncate font-semibold text-primary" :title="offer.pricePerCredit">
                  {{ offer.pricePerCredit ? '$' + offer.pricePerCredit : "" }}
                </span>
              </td>
              <td class="table-cell max-w-[8rem] overflow-hidden px-4 py-2.5 text-left">
                <a
              ...
              <td class="table-cell max-w-[8rem] overflow-hidden px-4 py-2.5 text-sm">
                ...
                <template v-else-if="offer.creditsRows && offer.creditsRows.length > 0">
                  ...
                  <button type="button" @click="showCreditsDetails(offer)" class="text-primary-600 hover:text-primary-700 hover:underline text-xs mt-1">Details</button>
                </template>
              </td>
              <td class="table-cell max-w-[10rem] overflow-hidden px-4 py-2.5 text-sm">
              ...
              <td class="table-cell max-w-[8rem] overflow-hidden px-4 py-2.5 text-sm">
                <template v-if="offer.defaultContractMode === 'upload'">
                  <a v-if="offer.defaultContractFileLink" :href="offer.defaultContractFileLink" target="_blank" rel="noopener noreferrer" class="text-primary-600 hover:text-primary-700 hover:underline">Contract File</a>
                </template>
                <template v-else-if="offer.defaultContractMode === 'template'">
                  <span v-if="offer.defaultContractTemplate === 'other'">{{ offer.defaultContractTemplateOther || "" }}</span>
                  <span v-else class="capitalize">{{ offer.defaultContractTemplate ? offer.defaultContractTemplate.replace('-', ' ') : "" }}</span>
                </template>
              </td>
              <td class="table-cell max-w-[10rem] overflow-hidden px-4 py-2.5 text-sm">
                <div class="flex flex-col gap-1 items-start">
                  <a v-if="offer.websiteLink" :href="offer.websiteLink" target="_blank" rel="noopener noreferrer" class="text-primary-600 hover:text-primary-700 hover:underline text-xs">Website</a>
                  <a v-if="offer.projectPdfLink" :href="offer.projectPdfLink" target="_blank" rel="noopener noreferrer" class="text-primary-600 hover:text-primary-700 hover:underline text-xs">Project PDF</a>
                  <a v-if="offer.dueDiligenceSite" :href="offer.dueDiligenceSite" target="_blank" rel="noopener noreferrer" class="text-primary-600 hover:text-primary-700 hover:underline text-xs">Due Diligence</a>
                  <a v-if="offer.vvbLink" :href="offer.vvbLink" target="_blank" rel="noopener noreferrer" class="text-primary-600 hover:text-primary-700 hover:underline text-xs">VVB Link</a>
                  <a v-else-if="offer.verifierVvbLink" :href="offer.verifierVvbLink" target="_blank" rel="noopener noreferrer" class="text-primary-600 hover:text-primary-700 hover:underline text-xs">VVB Link</a>
                </div>
              </td>
            </tr>
    ...
    <Dialog v-model:visible="detailsDialogVisible" modal header="Credits Details" :style="{ width: '30rem' }">
      <div v-if="selectedCreditsOffer" class="p-4">
        <h3 class="font-semibold text-lg mb-4">{{ selectedCreditsOffer.projectName }}</h3>
